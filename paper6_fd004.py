...
# -*- coding: utf-8 -*-
"""
Qwen3-0.6B-style model for RUL prediction on NASA CMAPSS FD004.

Idea:
- Use Qwen3-0.6B as a frozen backbone (feature extractor).
- Convert each multivariate time-series window into segment tokens (AutoTimes-style).
- Segment MLP -> Qwen hidden size.
- Add learnable position embeddings over tokens.
- Feed as `inputs_embeds` to Qwen3-0.6B.
- Last token representation -> regression head -> scalar RUL.

Training:
- 1 epoch, MSE loss, AdamW.
- Reports RMSE and MAE on validation and test sets.
...
Output:
    qwen3_0_6b_fd004_predictions.csv
"""
...
# Qwen3 backbone (you can switch to the Instruct variant if desired)
QWEN3_MODEL_NAME = "Qwen/Qwen3-0.6B"  # or "Qwen/Qwen3-0.6B-Instruct"

# Time-series and training hyperparameters
SEQ_LEN_RAW  = 60   # time steps per window
SEGMENT_LEN  = 10   # time steps per token (must divide SEQ_LEN_RAW)
BATCH_SIZE   = 16
...
# --------------------------------------------------------------------
# CMAPSS FD004 loading & preprocessing
# --------------------------------------------------------------------
def load_cmapss_fd004(path: str) -> pd.DataFrame:
    """
    Load CMAPSS FD004 file.
    ...
    # Some distributions include an extra empty column at the end
    if df.shape[1] == 27:
        df = df.iloc[:, :-1]
...
def add_rul_labels(train_df: pd.DataFrame) -> pd.DataFrame:
    """
    RUL = max_time(unit) - current_time for each unit.
    """
...
def load_rul_file(path: str) -> np.ndarray:
    """
    Load RUL_FD004.txt -> array of RUL values, one per test engine.
    """
...
def make_sequences_fd004(df: pd.DataFrame,
                         feature_cols,
                         seq_len_raw: int,
                         use_rul: bool = True):
    """
    Build sequences of fixed length for FD004.

    If use_rul=True (train/val):
        returns X (num_seq, seq_len_raw, num_features), y (num_seq,)
        Sliding windows; label is RUL at last time step of each window.

    If use_rul=False (test):
        returns X (num_units, seq_len_raw, num_features), unit_ids
        Use last seq_len_raw cycles per unit (pad at start if too short).
    """
    X_list = []
    y_list = []
    unit_ids = []
    ...
    for uid, group in df.groupby("unit"):
        ...
        if use_rul:
            ...
            # For test, take the last window per engine (pad with first row if needed)
            if len(group) >= seq_len_raw:
...
# --------------------------------------------------------------------
# Qwen3-0.6B AutoTimes-style RUL model
# --------------------------------------------------------------------
class Qwen3AutoTimesRUL(nn.Module):
    """
    AutoTimes-style model with Qwen3-0.6B as frozen backbone.

    - Split sequence into segment tokens: SEQ_LEN_RAW / SEGMENT_LEN tokens.
    - Each segment (segment_len × features) -> hidden_dim via MLP.
    - Add learnable position embeddings per token index.
    - Pass as `inputs_embeds` into Qwen3-0.6B (AutoModel).
    - Last token hidden state -> linear head -> scalar RUL.
    """
    ...
    def __init__(self,
                 ...
                 seq_len_raw: int,
                 segment_len: int,
                 model_name: str = QWEN3_MODEL_NAME):
        super().__init__()
        ...
        # Load Qwen3-0.6B backbone and freeze it
        self.qwen3 = AutoModel.from_pretrained(
            model_name,
            trust_remote_code=True
        )
        hidden_size = self.qwen3.config.hidden_size

        for p in self.qwen3.parameters():
            p.requires_grad = False

        # Segment embedding MLP: (segment_len * input_dim) -> hidden_size
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: (B, T_raw, F) with T_raw = seq_len_raw, F = input_dim.
        """
        B, T, F = x.shape
        ...
        # Flatten segments: (B, num_tokens, segment_len * F)
        ...
        # Pass through frozen Qwen3 backbone
        outputs = self.qwen3(inputs_embeds=token_embeds)
...
def main():
    ...
    print("Initializing Qwen3-0.6B AutoTimes-style model...")
    model = Qwen3AutoTimesRUL(
        input_dim=input_dim,
    ...
    # Align predictions with RUL_FD004 order
    ...
    out_path = os.path.join(DATA_DIR, "qwen3_0_6b_fd004_predictions.csv")
