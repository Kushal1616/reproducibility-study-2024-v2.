...
for RUL prediction on NASA CMAPSS FD003.
...
Pipeline:
- Load FD003 train/test, build RUL labels for train.
...
# --------------------------------------------------------------------
def load_cmapss_fd003(path):
    """
    Load CMAPSS FD003 train/test file with standard column names.
...
def load_rul_file(path):
    """
    Load RUL_FD003.txt -> np.array of RUL values, one per test engine.
    """
...
def make_sequences_fd003(df, feature_cols, seq_len_raw, use_rul=True):
    """
    Build sequences of fixed raw length for FD003.
...
def main():
    ...
    print("Loading FD003 train/test data...")
    ...
    X_train, y_train = make_sequences_fd003(
        train_df_split, feature_cols, SEQ_LEN_RAW, use_rul=True
    )
    X_val, y_val = make_sequences_fd003(
    ...
    X_test, test_units = make_sequences_fd003(
    ...
    # Align with RUL_FD003 ordering (unit id ascending)
