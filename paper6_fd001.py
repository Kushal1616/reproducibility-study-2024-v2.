...
# -*- coding: utf-8 -*-
"""
Qwen3-0.6B-style model for RUL prediction on NASA CMAPSS FD001.
...
Output:
    qwen3_0_6b_fd001_predictions.csv
"""
...
# --------------------------------------------------------------------
# CMAPSS FD001 loading & preprocessing
# --------------------------------------------------------------------
def load_cmapss_fd001(path: str) -> pd.DataFrame:
    """
    Load CMAPSS FD001 file.
...
def load_rul_file(path: str) -> np.ndarray:
    """
    Load RUL_FD001.txt -> array of RUL values, one per test engine.
    """
...
def make_sequences_fd001(df: pd.DataFrame,
                         feature_cols,
                         seq_len_raw: int,
                         use_rul: bool = True):
    """
    Build sequences of fixed length for FD001.
...
def main():
    ...
    test_df  = load_cmapss_fd001(TEST_FILE)
    ...
    # Align predictions with RUL_FD001 order
    ...
    out_path = os.path.join(DATA_DIR, "qwen3_0_6b_fd001_predictions.csv")
