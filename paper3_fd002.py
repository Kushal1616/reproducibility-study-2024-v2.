...
for RUL prediction on NASA CMAPSS FD002.
...
Pipeline:
- Load FD002 train/test, build RUL labels for train.
...
# --------------------------------------------------------------------
def load_cmapss_fd002(path):
    """
    Load CMAPSS FD002 train/test file with standard column names.
...
def load_rul_file(path):
    """
    Load RUL_FD002.txt -> np.array of RUL values, one per test engine.
    """
...
def make_sequences_fd002(df, feature_cols, seq_len_raw, use_rul=True):
    """
    Build sequences of fixed raw length for FD002.
