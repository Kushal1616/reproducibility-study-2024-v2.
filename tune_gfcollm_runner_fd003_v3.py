...
# -*- coding: utf-8 -*-
"""
Tune GF-CoLLM for CMAPSS FD003.

Design choice:
- FD003 is treated like FD001 (single operating condition), not like FD002/FD004.
- Start from the successful stability-first tuning recipe:
  * locked lr=2e-4
  * refinement cap controlled inside paper8_fd003.py
  * EMA validation
  * gate supervision + gate entropy
  * strong no-harm loss
- Run a small, sensible 3-config sweep around the FD001-best region.
"""
...
def get_feature_cols(train_df, fd=None):
    # FD003: same sensor-only choice as the successful FD001 pipeline
...
def train_gfcollm_with_schedule(p8, input_dim, train_loader, val_loader, device,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            # FD003 schedule: sharp selection, earlier heavy guardrail.
            if epoch <= 10:
                loss = (
                    1.5 * loss_s
                    + 1.0 * loss_l
...
def main():
    ...
    parser.add_argument("--out_csv", type=str, default="fd003_gfcollm_tuning.csv")
    args = parser.parse_args()

    set_seed(args.seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    p8 = importlib.import_module("paper8_fd003")
    ...
    # Small, sensible sweep around the successful region
    configs = [
        {"lr": 2e-4, "lambda_res": 0.3, "lambda_route": 0.1, "lambda_conf": 0.1, "lambda_no_harm_strong": 50.0},  # FD003 safe-switch config
    ]
