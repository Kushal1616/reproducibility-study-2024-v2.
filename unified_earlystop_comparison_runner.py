...
def train_p8_gfcollm_with_early_stopping(p8, input_dim, train_loader, val_loader, device, max_epochs=50, patience=7, fd=None):
    ...
    For FD002 / FD004, use a stability-first schedule:
    - longer branch-only warm-up
    - stronger no-harm penalty
    - gate entropy + gate supervision when available
    - EMA validation if the p8 module exposes EMA

    For FD001 / FD003, keep a conservative guarded schedule.
    """
    ...
    # Optional EMA support if the p8 module exposes an EMA helper.
    ema = None
    if hasattr(p8, "EMA"):
        try:
    ...
    def set_trainable_by_name(epoch_now):
        # Foundation-first for FD002 / FD004
        if fd in {"FD002", "FD004"}:
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            # Optional heads depending on the model file version
            ...
            with torch.no_grad():
                ...
                per_sample_err_best_branch = torch.min(branch_errs, dim=1)[0]
            ...
            if gate_logits is not None:
                ...
                # Fall back to pseudo-logits from probabilities
            ...
            # Optional residual refinement loss if exposed
            loss_resid_refine = torch.tensor(0.0, device=device)
            if "residual_hat" in out:
                loss_resid_refine = torch.mean(out["residual_hat"] ** 2)
            ...
            # FD-aware schedule
            if fd in {"FD002", "FD004"}:
                # Longer foundation stage for multi-condition settings
                if epoch <= 25:
                    loss = (
                        1.0 * loss_s
                        + 1.0 * loss_l
