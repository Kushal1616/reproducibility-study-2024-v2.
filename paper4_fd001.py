...
# -*- coding: utf-8 -*-
"""
AutoTimes-style model for RUL prediction on NASA CMAPSS FD001.
...
Difference vs original AutoTimes paper:
- Here we adapt the architecture to *regress RUL* (scalar) instead of forecasting
  future segments. The last token representation is used for RUL regression.
...
- Saves test predictions to `autotimes_fd001_predictions.csv`.
"""
...
class AutoTimesRUL(nn.Module):
    ...
    def __init__(self,
        ...
        # Learnable position embeddings for tokens (AutoTimes uses textual timestamps;
        # here we approximate with standard learnable embeddings for simplicity).
...
def main():
    ...
    out_path = os.path.join(DATA_DIR, "autotimes_fd001_predictions.csv")
