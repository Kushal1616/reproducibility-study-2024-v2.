...
# -*- coding: utf-8 -*-
"""
GF-CoLLM on CMAPSS FD002 with:
- expert-gating fusion over small / large / corrected branches (FD002 condition-aware)
...
# FD002 stability-first parameters:
# LR=2e-4, LAMBDA_RESIDUAL=0.2, LAMBDA_ROUTE=0.1, LAMBDA_CONF=0.1
# Keep refinement cap tight (±2), let FIG see operating conditions,
# and rely on the external tuning runner for stronger no-harm scheduling.
...
# -----------------------------------------------------------
# Data loading utilities for FD002
# -----------------------------------------------------------

def load_fd002(path: str) -> pd.DataFrame:
...
class FIGLayer(nn.Module):
    """
    FD002-aware FIG layer:
    - first 3 features are assumed to be operating conditions: os1, os2, os3
    ...
    def forward(self, x):
        ...
        # Operating conditions are effectively regime labels; summarizing them gives
        # the gate a direct signal about the current flight/engine condition.
...
def main():
    ...
    train_df = load_fd002(TRAIN_FILE)
    test_df = load_fd002(TEST_FILE)
    ...
    # FD002: put operating conditions first so the FIG layer can treat them explicitly.
    ...
    print("Evaluating enhanced GF-CoLLM on FD002 test set...")
