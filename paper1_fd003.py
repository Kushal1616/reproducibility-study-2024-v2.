...
# -*- coding: utf-8 -*-
"""
GPT-2 fine-tuning for RUL prediction on NASA CMAPSS FD003.
...
Pipeline:
- Load FD003 train/test and build RUL labels for train.
...
- Save test predictions to: gpt2_predictions_FD003.csv
"""
...
def load_fd003(path):
    """
    Load FD003 file (train or test) as DataFrame with proper column names.
...
def add_rul_labels(train_df):
    """
    Add Remaining Useful Life (RUL) label for each row in FD003 training data:
...
def load_rul_file(path):
    """
    Load RUL_FD003.txt: each row k contains the true RUL at the last cycle
    of the k-th test engine.
    """
    rul_df = pd.read_csv(path, sep=r"\s+", header=None, engine="python")
    # Remove possible empty last column
    if rul_df.shape[1] > 1:
...
def main():
    ...
    # 13) Prepare test sequences and evaluate on test set using RUL_FD003
    ...
    # FD003 RUL file is assumed to be in ascending unit_id order (1..num_units).
