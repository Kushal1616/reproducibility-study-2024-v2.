...
# -*- coding: utf-8 -*-
"""
FD004 clean reviewer-table script using the tuned GF-CoLLM training schedule.
...
def maybe_make_split_json(train_df: pd.DataFrame, split_path: str, seed: int = 42):
    ...
    split = {"train_engine_ids": train_ids, "val_engine_ids": val_ids}
...
def infer_p8_module():
    try:
        return importlib.import_module("paper8_fd004")
    except Exception:
        pass
    candidates = ["paper8_fd004.py", "paper8_fd004 (3).py", "paper8_fd004 (2).py"]
    for fp in candidates:
        if os.path.exists(fp):
            return import_module_from_file("paper8_fd004_dynamic", fp)
    raise FileNotFoundError(
        "Could not find paper8_fd004 model file. Expected one of: "
        "paper8_fd004.py, paper8_fd004 (3).py, paper8_fd004 (2).py"
    )
...
def load_raw_data_with_p8(p8, data_dir, fd):
    ...
    load_fn = None
    for name in [f"load_{fd.lower()}", "load_fd004", "load_fd001", "load_cmapss"]:
        if hasattr(p8, name):
...
def create_windows_trainvaltest(train_norm, val_norm, test_norm, test_rul_true, feature_cols, window_size):
...
def train_gfcollm_with_schedule(p8, input_dim, train_loader, val_loader, device,
    ...
    print("FD004 tuner v12 ACTIVE: dual-expert fusion (Large vs Corrected) + gate-weight logging")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            loss_diversity = torch.mean(torch.exp(-torch.abs(y_l - y_c)))

            if epoch <= 20:
                loss = 1.2 * loss_s + 1.0 * loss_l + 0.5 * loss_res + 0.05 * loss_ortho + 0.05 * loss_diversity
            elif epoch <= 30:
                loss = (
                    loss_pred + 0.5 * loss_res + 0.6 * (loss_s + loss_l) + 5.0 * loss_no_harm
                    + lambda_res * loss_resid_refine + 0.5 * loss_gate_supervision + 0.01 * gate_entropy
                    + 0.05 * loss_ortho + 0.05 * loss_diversity
                )
            else:
                lambda_no_harm = lambda_no_harm_strong
                loss = (
                    loss_pred + 0.5 * loss_res + lambda_no_harm * loss_no_harm
                    + lambda_res * loss_resid_refine + lambda_route * loss_route + lambda_conf * loss_conf
                    + 1.0 * loss_gate_supervision + 0.01 * gate_entropy + 0.05 * loss_ortho + 0.10 * loss_diversity
                )
        ...
        print(f"[Epoch {epoch:02d}] train_obj={train_obj:.4f} val_fused_RMSE={val_metrics['f_rmse']:.4f} (s={val_metrics['s_rmse']:.4f}, l={val_metrics['l_rmse']:.4f}, c={val_metrics['c_rmse']:.4f})")
...
def infer_outputs(model, loader, device):
    model.eval()
    ys_true = []
    merged = {"y_s": [], "y_l": [], "y_c": [], "y_fused": [], "route_logits": [], "gate_weights": [], "conf_s": [], "conf_l": [], "conf_score": []}
    with torch.no_grad():
        for xb, yb in loader:
            xb = xb.to(device)
            out = model(xb)
            ...
            ys_true.append(yb.numpy())
    y_true = np.concatenate(ys_true, axis=0)
    merged = {k: np.concatenate(v, axis=0) for k, v in merged.items() if len(v) > 0}
...
def predict_by_mode(mode: str, outputs: Dict[str, np.ndarray]):
    ...
    y_c = outputs["y_c"]
    y_f = outputs["y_fused"]
    conf_s = outputs.get("conf_s", np.ones_like(y_s))
    conf_l = outputs.get("conf_l", np.ones_like(y_s))
    conf_score = outputs.get("conf_score", np.zeros_like(y_s))
    route_logits = outputs.get("route_logits", None)
    gate_weights = outputs.get("gate_weights", None)
    ...
    if mode == "small_only":
        ...
        if gate_weights is not None and len(gate_weights.shape) == 2 and gate_weights.shape[1] == 2:
            gate_idx = np.argmax(gate_weights, axis=1)
            pred = np.where(gate_idx == 0, y_l, y_c)
...
def measure_mode_latency(model, loader, device, mode: str):
    ...
    with torch.no_grad():
        for xb, _ in loader:
            ...
            out = model(xb)
            out_np = {}
            for k in ["y_s", "y_l", "y_c", "y_fused", "route_logits", "gate_weights", "conf_s", "conf_l", "conf_score"]:
                if k in out:
                    out_np[k] = out[k].detach().cpu().numpy()
            _ = predict_by_mode(mode, out_np)
            if device.type == "cuda":
                torch.cuda.synchronize()
...
def main():
    ...
    X_train, y_train, X_val, y_val, X_test, y_test = create_windows_trainvaltest(train_norm, val_norm, test_norm, test_rul_true, feature_cols, window_size)
    train_loader, val_loader, test_loader = make_loaders(X_train, y_train, X_val, y_val, X_test, y_test, batch_size)

    cfg = {"lr": 2e-4, "lambda_res": 0.2, "lambda_route": 0.1, "lambda_conf": 0.1, "lambda_no_harm_strong": 40.0}
    print(f"Training tuned FD004 GF-CoLLM with config: {cfg}")
    ...
    modes = ["small_only", "corrected_large_side", "threshold_routing", "mlp_gate", "type1_fuzzy_routing", "it2_fuzzy_routing", "gf_collm"]
    ...
    os.makedirs("fd004_branch_value_outputs", exist_ok=True)
    out_csv = "fd004_branch_value_outputs/fd004_branch_value_table_clean.csv"
    out_df.to_csv(out_csv, index=False)

    print("\n=== Clean FD004 Branch-Value Table ===")
