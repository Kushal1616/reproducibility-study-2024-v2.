...
# -*- coding: utf-8 -*-
"""
CoLLM-C (approximate implementation) on CMAPSS FD001.
...
- CoLLM-C thresholds for FD001: tau1 = 0.6, tau2 = 0.05
...
Output:
- Prints RMSE and MAE on FD001 test set (engine-level evaluation)
"""
...
# CoLLM-C thresholds for FD001 (tau1, tau2)
TAU1 = 0.6
TAU2 = 0.05

ALPHA_CONF = 10.0       # scaling factor in tanh for confidence labels
...
def load_rul_file(path: str) -> np.ndarray:
    """
    RUL_FD001.txt -> array of size (#test_units,)
    """
...
class PatchTransformerL(nn.Module):
    ...
    Patch size & stride from paper's hyperparameters (Table I).
    """
    ...
    def forward(self, x):
        ...
        tokens = tokens.transpose(0, 1)          # (S, B, E) for Transformer
...
class FuzzyDecisionAgent(nn.Module):
    ...
    def __init__(self,
        ...
        # Learnable Gaussian parameters mu and sigma for each membership and feature
...
def train_stage3_fuzzy_and_reflection(model_s, model_l,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for X_batch, y_batch in train_loader:
            ...
            # Forward through frozen S and L
            with torch.no_grad():
                y_s, feat_s = model_s(X_batch)
                y_l, feat_l = model_l(X_batch)

            # Confidence labels (Eq. 10 and 13 style)
...
def collm_forward_batch(model_s, model_l, agent_f, reflect_r,
    ...
    with torch.no_grad():
        ...
        # Decide which samples need large model
        need_large = Q_s < tau1

        # Initialize final prediction with small model outputs
        y_final = y_s.clone()

        if need_large.any():
            # Run large model only once for whole batch
            y_l, feat_l = model_l(X_batch)
            Q_l = reflect_r(feat_l)

            delta = Q_s - Q_l

            # Among those needing large model, decide between LM or fusion
...
def main():
    ...
    # Use 14 informative sensors (paper removes sensors 1,5,6,10,16,18,19)
    ...
    # Create test last windows (per engine)
    ...
    # -------------------------------------------------------
    # Final evaluation on test set (engine-level)
    # -------------------------------------------------------
    print("=== Final CoLLM-C Evaluation on FD001 Test ===")
