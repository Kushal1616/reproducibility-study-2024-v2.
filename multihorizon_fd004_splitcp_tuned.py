...
# -*- coding: utf-8 -*-
"""
FD004 multi-horizon CMAPSS baselines with Split Conformal Prediction (Split CP).
...
FD004 tuning choices
--------------------
- operating conditions + informative sensors
...
- GF-CoLLM schedule adapted from the FD004 dual-expert recipe:
  * fusion frozen until epoch 25
  * dual-expert final fusion over Large and Corrected only
  * strong powered no-harm loss
  * gate supervision over [Large, Corrected]
"""
...
def build_multihorizon_windows(df: pd.DataFrame, feature_cols: List[str], window_size: int, horizon: int,
                               unit_ids: Optional[List[int]] = None) -> Tuple[np.ndarray, np.ndarray]:
    X_list, Y_list = [], []
...
def train_p7_collm_with_early_stopping(p7, input_dim: int, train_loader: DataLoader, val_loader: DataLoader,
    ...
    for epoch in range(1, e3 + 1):
        ...
        for xb, yb in train_loader:
            ...
            y_collab = mu * y_s + (1.0 - mu) * (y_l + refl)
            loss = nn.MSELoss()(y_collab, yb); loss.backward(); opt.step()
...
def train_p8_gfcollm_fd004_with_early_stopping(p8, input_dim: int, train_loader: DataLoader, val_loader: DataLoader,
                                               device: str, max_epochs: int = 50, patience: int = 7):
    model = p8.GFCoLLM(input_dim=input_dim, window_size=p8.WINDOW_SIZE).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=getattr(p8, "LR", 2e-4))
    ...
    def eval_val():
        if hasattr(p8, "evaluate_gf_collm"):
            if ema is not None:
                try:
                    ema.apply_shadow(model)
                    res = p8.evaluate_gf_collm(model, val_loader, device)
                    ema.restore(model)
                except Exception:
                    res = p8.evaluate_gf_collm(model, val_loader, device)
            else:
                res = p8.evaluate_gf_collm(model, val_loader, device)
            if isinstance(res, dict) and "fused" in res and isinstance(res["fused"], dict):
                return float(res["fused"]["rmse"])
        ...
        with torch.no_grad():
            for xb, yb in val_loader:
                ...
                preds.append(out["y_fused"].cpu().numpy()); trues.append(yb.numpy())
        p = np.concatenate(preds); t = np.concatenate(trues)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            with torch.no_grad():
                ...
                err_l = (y_l - yb) ** 2
                err_c = (y_c - yb) ** 2
                all_errs = torch.stack([err_s, err_l, err_c], dim=1)
                best_idx_all = torch.argmin(all_errs, dim=1)
                best_err = torch.min(all_errs, dim=1)[0]
                gate_errs = torch.stack([err_l, err_c], dim=1)
                best_idx_gate = torch.argmin(gate_errs, dim=1)
                fused_err = (y_fused - yb) ** 2

            loss_s = mse(y_s, yb)
            loss_l = mse(y_l, yb)
            loss_pred = mse(y_fused, yb)
            loss_res = mse(y_c, yb)
            loss_no_harm = torch.mean(torch.relu(fused_err - best_err) ** 2)
            loss_route = ce(route_logits, best_idx_all) if route_logits is not None else torch.tensor(0.0, device=device)
            loss_conf = torch.mean((conf_score - torch.abs(y_fused - yb)) ** 2) if conf_score is not None else torch.tensor(0.0, device=device)
            loss_gate_supervision = ce(gate_logits, best_idx_gate) if gate_logits is not None else torch.tensor(0.0, device=device)
            gate_entropy = -torch.mean(torch.sum(gate_weights * torch.log(gate_weights + 1e-6), dim=-1)) if gate_weights is not None else torch.tensor(0.0, device=device)
            loss_resid_refine = torch.mean(residual_hat ** 2)

            if epoch <= 25:
                loss = 1.0 * loss_s + 1.0 * loss_l
            elif epoch <= 35:
                loss = loss_pred + 0.5 * loss_res + 0.8 * (loss_s + loss_l) + 2.0 * loss_no_harm + 0.2 * loss_resid_refine + 0.25 * loss_gate_supervision + 0.01 * gate_entropy
            else:
                loss = loss_pred + 0.5 * loss_res + 20.0 * loss_no_harm + 0.2 * loss_resid_refine + 0.1 * loss_route + 0.1 * loss_conf + 0.5 * loss_gate_supervision + 0.01 * gate_entropy
        ...
        val_rmse = eval_val()
        print(f"[Epoch {epoch:02d}] val_fused_RMSE={val_rmse:.4f}")
        if val_rmse < best_val:
            best_val = val_rmse; best_state = copy.deepcopy(model.state_dict()); patience_ctr = 0
        else:
            patience_ctr += 1
        if patience_ctr >= patience:
            print(f"Early stopping at epoch {epoch}")
...
def tune_and_fit_gru(input_dim, train_loader, val_loader, device, max_epochs, patience):
    ...
    best_model=None; best_val=float("inf")
    for cfg in search:
        print(f"[GRU tuning] trying {cfg}")
        model = GRURegressor(input_dim=input_dim, hidden_dim=cfg["hidden_dim"], num_layers=cfg["num_layers"])
        model, val_rmse = train_simple_with_early_stopping(model, train_loader, val_loader, device, cfg["lr"], max_epochs, patience)
        if val_rmse < best_val:
...
def tune_and_fit_transformer(input_dim, train_loader, val_loader, device, max_epochs, patience):
    ...
    for cfg in search:
        ...
        model = TimeSeriesTransformer(input_dim=input_dim, d_model=cfg["d_model"], nhead=cfg["nhead"], num_layers=cfg["num_layers"], ff_dim=cfg["ff_dim"])
        model, val_rmse = train_simple_with_early_stopping(model, train_loader, val_loader, device, cfg["lr"], max_epochs, patience)
        if val_rmse < best_val:
            best_model, best_val = model, val_rmse
    return best_model, best_val
...
def main():
    ...
    parser.add_argument("--out_csv", type=str, default="fd004_multihorizon_splitcp_summary.csv")
    ...
    if args.fd.upper() != "FD004":
        raise ValueError("This script is for FD004 only.")

    train_path = os.path.join(args.data_dir, "train_FD004.txt")
    test_path = os.path.join(args.data_dir, "test_FD004.txt")
    rul_path = os.path.join(args.data_dir, "RUL_FD004.txt")
    ...
    if "CoLLM-C" in args.methods:
        for name in ["paper7_fd004", "paper7_fd003", "paper7_fd002", "paper7_fd001"]:
            try:
                p7 = maybe_import_module(name)
                print(f"[INFO] Using {name} for CoLLM-C")
                break
            except Exception:
                continue
        if p7 is None:
            raise ImportError("Could not import any paper7 module for FD004")

    p8 = maybe_import_module("paper8_fd004") if "GF-CoLLM" in args.methods else None
    ...
    for method in args.methods:
        print(f"\n===== Method: {method} | Dataset: FD004 | H={args.horizon} =====")
        all_test_preds, all_test_true, all_q = [], [], []

        for h in range(args.horizon):
            y_train_h = Y_train[:, h]; y_val_h = Y_val[:, h]; y_test_h = Y_test[:, h]
            train_loader, val_loader, test_loader = make_scalar_loaders(X_train, y_train_h, X_val, y_val_h, X_test, y_test_h, args.batch_size)
            ...
            if method == "GRU":
                ...
                model_s, model_l, agent_f, reflect_r, best_val = train_p7_collm_with_early_stopping(p7, input_dim, train_loader, val_loader, device, args.max_epochs, args.patience)
                val_pred, val_true = predict_collmc_point(model_s, model_l, agent_f, reflect_r, val_loader, device)
                test_pred, test_true = predict_collmc_point(model_s, model_l, agent_f, reflect_r, test_loader, device)
            elif method == "GF-CoLLM":
                model, best_val = train_p8_gfcollm_fd004_with_early_stopping(p8, input_dim, train_loader, val_loader, device, args.max_epochs, args.patience)
            ...
            detail_rows.append({
                "Method": method, "Dataset": "FD004", "Horizon": args.horizon, "Step": h + 1,
            ...
            if args.save_npz_dir:
                fname = f"FD004_{method.replace(' ', '_').replace('+', 'plus').replace('-', '_')}_h{h+1}.npz"
        ...
        avg_rmse = float(np.mean([np.sqrt(mean_squared_error(test_true_mat[:, i], test_pred_mat[:, i])) for i in range(test_true_mat.shape[1])]))
        ...
        summary_rows.append({
            "Method": method, "Dataset": "FD004", "Horizon": args.horizon,
