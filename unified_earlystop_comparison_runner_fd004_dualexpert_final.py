...
def infer_fd_modules(fd):
    ...
    For FD004, if paper7_fd004.py is unavailable, fall back to the closest
    available paper7 baseline module instead of crashing.
    """
    fd = fd.upper()
    ...
    p7_candidates = {
        "FD001": ["paper7_fd001"],
        "FD002": ["paper7_fd002", "paper7_fd001"],
        "FD003": ["paper7_fd003", "paper7_fd001"],
        "FD004": ["paper7_fd004", "paper7_fd003", "paper7_fd002", "paper7_fd001"],
    }
    ...
    for p7_name in p7_candidates[fd]:
        try:
    ...
    if p7 is None:
        raise ModuleNotFoundError(
            f"Could not import any paper7 module for {fd}. Tried: {p7_candidates[fd]}"
        ) from last_err

    p8 = importlib.import_module(p8_name)
    print(f"[Module Loader] Using {chosen_p7}.py and {p8_name}.py")
    return p7, p8
...
def train_p8_gfcollm_with_early_stopping(p8, input_dim, train_loader, val_loader, device, max_epochs=50, patience=7, fd=None):
    ...
    FD004 uses the validated dual-expert recipe:
    - final fusion only between Large and Corrected
    - route head may still observe all three branches
    - gate supervision is only over [Large, Corrected]
    - powered no-harm loss for safety
    """
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            with torch.no_grad():
                ...
                # 1) Best across ALL branches (route supervision and no-harm reference)
