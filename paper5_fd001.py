...
# -*- coding: utf-8 -*-
"""
Qwen2.5-0.5B-style model for RUL prediction on NASA CMAPSS FD001.

Architecture idea (aligned with the Qwen2.5 technical report):
- Use the dense Transformer decoder backbone (Qwen2.5-0.5B) as a frozen feature extractor.
- Build segment-based "tokens" along the time axis from multivariate sensor sequences.
- A small trainable MLP maps each segment to the Qwen hidden dimension (segment embedding).
- Optional learnable token-wise position embeddings.
- Feed these embeddings as `inputs_embeds` to the frozen Qwen2.5-0.5B backbone.
- Take the last token representation and pass it through a small regression head for scalar RUL.

Training:
- 1 epoch, MSE loss, AdamW optimizer.
- Reports RMSE & MAE on validation and test sets.
...
- Saves test predictions to `qwen25_fd001_predictions.csv`.
"""
...
# Qwen2.5 backbone (change name if your local HF id differs)
QWEN_MODEL_NAME = "Qwen/Qwen2.5-0.5B"  # or "Qwen/Qwen2.5-0.5B-Instruct"
...
def load_cmapss_fd001(path: str) -> pd.DataFrame:
    """
    Load CMAPSS FD001 train/test file.
...
# --------------------------------------------------------------------
# Qwen2.5-0.5B–style AutoTimes RUL model
# --------------------------------------------------------------------
class QwenAutoTimesRUL(nn.Module):
    """
    AutoTimes-style model adapted to use Qwen2.5-0.5B as frozen backbone for RUL regression.

    - Segment-based tokens along time axis:
        Each token is a contiguous segment of length SEGMENT_LEN over all features.
    - SegmentEmbedding (MLP) maps each segment to Qwen hidden dimension.
    - Learnable position embeddings per token index.
    - Frozen Qwen2.5-0.5B processes the token sequence via `inputs_embeds`.
    ...
    def __init__(self,
        ...
        # Load Qwen2.5-0.5B backbone and freeze it
        ...
        # Learnable position embeddings for tokens (optional, in addition to RoPE inside Qwen)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Feed into Qwen via inputs_embeds (backbone is frozen)
        # Qwen is decoder-only; we only need the last_hidden_state
        outputs = self.qwen(inputs_embeds=token_embeds)
...
def main():
    ...
    print("Initializing Qwen2.5-0.5B–style AutoTimes model...")
    model = QwenAutoTimesRUL(
        input_dim=input_dim,
    ...
    out_path = os.path.join(DATA_DIR, "qwen25_fd001_predictions.csv")
