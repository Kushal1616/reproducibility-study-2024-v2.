...
# This version uses a much harder gate, a tighter refinement cap,
# disagreement-aware gating inputs, and hard-thresholded safe-switch gating.
...
# -----------------------------------------------------------
# Data loading utilities for FD003
# -----------------------------------------------------------
...
class SharpenedExpertGating(nn.Module):
    ...
    def forward(self, y_s, y_l, y_c, g, conf_s, conf_l):
        ...
        d_sl = torch.abs(y_s - y_l).unsqueeze(-1)
        d_lc = torch.abs(y_l - y_c).unsqueeze(-1)
        d_cs = torch.abs(y_c - y_s).unsqueeze(-1)

        gate_in = torch.cat([g, conf_s.unsqueeze(-1), conf_l.unsqueeze(-1), d_sl, d_lc, d_cs], dim=-1)
        ...
        refine = torch.tanh(self.refine_net(g).squeeze(-1)) * 1.0
...
class GFCoLLM(nn.Module):
    def __init__(self, input_dim: int, window_size: int):
        ...
        self.fusion = SharpenedExpertGating(granule_dim=granule_dim, temperature=0.1, refine_scale=1.0)
...
def main():
    ...
    print("Evaluating enhanced GF-CoLLM on FD003 test set...")
