...
# -*- coding: utf-8 -*-
"""
FD002 multi-horizon CMAPSS baselines with Split Conformal Prediction (Split CP).
...
Key FD002 design choices
------------------------
- Uses operating conditions + informative sensors, matching the FD002-aware setup.
- Uses direct multi-horizon training: one scalar predictor per horizon.
- Uses train/val engine split JSON for both early stopping and conformal calibration.
- Applies FD002-specific GF-CoLLM tuning/schedule adapted from the attached tuning runner:
  * locked lr = 2e-4
  * long warm-up (fusion frozen until epoch 25)
  * EMA validation
  * stronger no-harm penalty
  * gate supervision + gate entropy
  * operating-condition-aware feature set

Expected external modules
-------------------------
- paper7_fd002.py for CoLLM-C
- paper8_fd002.py for GF-CoLLM
"""
...
def evaluate_components(model, loader, device):
    ...
    return {"s_rmse": rmse(y_true, y_s), "l_rmse": rmse(y_true, y_l),
            "c_rmse": rmse(y_true, y_c), "f_rmse": rmse(y_true, y_f), "f_mae": mae(y_true, y_f)}
...
def train_p8_gfcollm_fd002_with_early_stopping(p8, input_dim: int, train_loader: DataLoader, val_loader: DataLoader,
                                               device: str, max_epochs: int = 50, patience: int = 7):
    ...
    best_state=None; best_val_rmse=float("inf"); patience_ctr=0

    print("FD002 stability tuner: condition-aware FIG + refine cap ±2 + EMA validation + locked lr=2e-4 + lambda_entropy=0.01 + lambda_no_harm=20 + 25-epoch warm-up")
    for epoch in range(1, max_epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            feat_s = out["feat_s"]; feat_l_proj = out["feat_l_proj"]; residual_hat = out["residual_hat"]
            route_logits = out["route_logits"]; conf_score = out["conf_score"]
            gate_weights = out["gate_weights"]; gate_logits = out["gate_logits"]

            loss_s = mse(y_s, yb); loss_l = mse(y_l, yb)
            ...
            loss_pred = mse(y_fused, yb); loss_res = mse(y_c, yb); loss_resid_refine = mse(residual_hat, (yb - y_l))

            with torch.no_grad():
                errs = torch.stack([(y_s - yb) ** 2, (y_l - yb) ** 2, (y_c - yb) ** 2], dim=1)
            ...
            branch_sq_errs = torch.stack([(y_s - yb) ** 2, (y_l - yb) ** 2, (y_c - yb) ** 2], dim=1)
            ...
            if epoch <= 25:
                ...
                loss = (loss_pred + 0.5 * loss_res + 0.8 * (loss_s + loss_l) + 2.0 * loss_no_harm
                        + 0.2 * loss_resid_refine + 0.25 * loss_gate_supervision + 0.01 * gate_entropy
                        + 0.1 * loss_ortho + 0.05 * loss_diversity)
            else:
                loss = (loss_pred + 0.5 * loss_res + 20.0 * loss_no_harm + 0.2 * loss_resid_refine
        ...
        if val_metrics["f_rmse"] < best_val_rmse:
            best_val_rmse = val_metrics["f_rmse"]; best_state = copy.deepcopy(model.state_dict()); patience_ctr = 0
        else:
            patience_ctr += 1
        ema.restore(model)
        if patience_ctr >= patience:
...
def main():
    ...
    parser.add_argument("--fd", type=str, default="FD002")
    parser.add_argument("--data_dir", type=str, default="FD002")
    ...
    parser.add_argument("--out_csv", type=str, default="fd002_multihorizon_splitcp_summary.csv")
    ...
    if args.fd.upper() != "FD002":
        raise ValueError("This script is for FD002 only.")

    train_path = os.path.join(args.data_dir, "train_FD002.txt")
    test_path = os.path.join(args.data_dir, "test_FD002.txt")
    rul_path = os.path.join(args.data_dir, "RUL_FD002.txt")
    ...
    p7 = maybe_import_module("paper7_fd002") if "CoLLM-C" in args.methods else None
    p8 = maybe_import_module("paper8_fd002") if "GF-CoLLM" in args.methods else None
    ...
    for method in args.methods:
        print(f"\n===== Method: {method} | Dataset: FD002 | H={args.horizon} =====")
        ...
        for h in range(args.horizon):
            ...
            if method == "GRU":
                ...
                model, best_val = train_p8_gfcollm_fd002_with_early_stopping(
            ...
            detail_rows.append({
                "Method": method, "Dataset": "FD002", "Horizon": args.horizon, "Step": h + 1,
            ...
            if args.save_npz_dir:
                fname = f"FD002_{method.replace(' ', '_').replace('+', 'plus').replace('-', '_')}_h{h+1}.npz"
        ...
        summary_rows.append({
            "Method": method, "Dataset": "FD002", "Horizon": args.horizon,
