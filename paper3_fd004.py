...
for RUL prediction on NASA CMAPSS FD004.

Implements key ideas from "One Fits All: Power General Time Series Analysis
by Pretrained LM" (frozen GPT2 backbone, patching, RevIN, linear embedding).
...
# --------------------------------------------------------------------
def load_cmapss_fd004(path):
    """
    Load CMAPSS FD004 train/test file with standard column names.
...
def load_rul_file(path):
    """
    Load RUL_FD004.txt -> np.array of RUL values, one per test engine.
    """
...
def make_sequences_fd004(df, feature_cols, seq_len_raw, use_rul=True):
    """
    Build sequences of fixed raw length for FD004.
...
class OneFitsAllFPT(nn.Module):
    ...
    def __init__(self, input_dim, seq_len_raw, patch_len, gpt2_name=GPT2_BACKBONE):
        ...
        # Unfreeze LayerNorms and positional embeddings (as in FPT)
...
def main():
    ...
    train_df = load_cmapss_fd004(TRAIN_FILE)
    test_df  = load_cmapss_fd004(TEST_FILE)
    ...
    X_train, y_train = make_sequences_fd004(
        train_df_split, feature_cols, SEQ_LEN_RAW, use_rul=True
    )
    X_val, y_val = make_sequences_fd004(
    ...
    X_test, test_units = make_sequences_fd004(
