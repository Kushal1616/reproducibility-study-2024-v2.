...
# -*- coding: utf-8 -*-
"""
Corrected FD001 reviewer-table script using the tuned GF-CoLLM training schedule.

Key fixes versus the earlier version
------------------------------------
1. Imports mean_squared_error explicitly.
2. Defines threshold / type-1 / IT2 fuzzy routing against the "large-side"
   prediction y_c (corrected large branch), not raw y_l.
3. Keeps the tuned training schedule from the FD001 tuning runner:
   EMA + early stopping + fusion warm-up + no-harm + gate supervision.

This cleaned version reports only the reviewer-safe columns:
- model_routing_mode
- rmse
- mae
- coverage
...
FD = "FD001"
DATA_DIR = "FD001"
P8_MODULE_NAME = "paper8_fd001"
SPLIT_JSON = "fd001_split.json"
...
def infer_p8_module():
    return importlib.import_module(P8_MODULE_NAME)
...
def train_gfcollm_with_schedule(
    ...
    print("TUNER V7 ACTIVE: EMA + no-harm + gate supervision + fusion warm-up")
...
def infer_outputs(model, loader, device, with_timing=False):
    model.eval()
    ys_true = []
    merged = {
        "y_s": [], "y_l": [], "y_c": [], "y_fused": [],
    ...
    latency_ms_per_sample = 1000.0 * total_time / max(1, total_samples) if with_timing else 0.0
    return y_true, merged, latency_ms_per_sample
...
def large_side_conf(conf_l, conf_score):
    # Encourage corrected-large selection when large-side uncertainty is low.
...
def measure_mode_latency(model, loader, device, mode: str):
    # True per-mode latency: forward pass + routing decision logic
    model.eval()
    total_time = 0.0
    total_samples = 0
    with torch.no_grad():
...
def main():
    ...
    print(f"Training tuned FD001 GF-CoLLM with config: {cfg}")
    ...
    val_y, val_outputs, _ = infer_outputs(model, val_loader, device, with_timing=False)
    test_y, test_outputs, _ = infer_outputs(model, test_loader, device, with_timing=False)
    ...
    os.makedirs("fd001_branch_value_outputs", exist_ok=True)
    out_csv = "fd001_branch_value_outputs/fd001_branch_value_table_clean.csv"
    out_df.to_csv(out_csv, index=False)

    print("\n=== Clean FD001 Branch-Value Table ===")
