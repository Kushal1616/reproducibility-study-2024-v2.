...
# -*- coding: utf-8 -*-
"""
Qwen2.5-0.5B-style model for RUL prediction on NASA CMAPSS FD004.

Architecture:
- Use Qwen2.5-0.5B decoder as frozen backbone.
- Turn multivariate time series into segment-based tokens (AutoTimes-style).
- Segment MLP → Qwen hidden dimension.
- Learnable position embeddings over tokens.
- Frozen Qwen processes token sequence via inputs_embeds.
...
# Qwen2.5 backbone (change if your HF id is different)
QWEN_MODEL_NAME = "Qwen/Qwen2.5-0.5B"  # or "Qwen/Qwen2.5-0.5B-Instruct"

# Time-series specifics
SEQ_LEN_RAW  = 60   # history window
SEGMENT_LEN  = 10   # time steps per token
BATCH_SIZE   = 16
...
def load_cmapss_fd004(path: str) -> pd.DataFrame:
    """
    Load CMAPSS FD004 train/test file.
    ...
    # Some releases have an extra empty column
    if df.shape[1] == 27:
        df = df.iloc[:, :-1]
...
def make_sequences_fd004(df: pd.DataFrame,
    ...
    If use_rul=True (train/val):
        ...
        Sliding windows; label is RUL at last time step in each window.
...
# --------------------------------------------------------------------
# Qwen2.5-0.5B AutoTimes-style RUL model
# --------------------------------------------------------------------
class QwenAutoTimesRUL(nn.Module):
    """
    AutoTimes-style model adapted to use Qwen2.5-0.5B as frozen backbone.
    ...
    def __init__(self,
        ...
        # SegmentEmbedding: flatten segment (segment_len * features) -> hidden_size
        ...
        # Learnable position embeddings for tokens (on top of Qwen’s internal RoPE)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Token embeddings = segment embedding + position embedding
...
def main():
    ...
    # Only segment_embedding, pos_emb, and reg_head are trainable
    ...
    out_path = os.path.join(DATA_DIR, "qwen25_fd004_predictions.csv")
