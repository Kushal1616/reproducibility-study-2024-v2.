...
# FD003 fusion-stability starting parameters:
# LR=2e-4, LAMBDA_RESIDUAL=0.2, LAMBDA_ROUTE=0.1, LAMBDA_CONF=0.1
# FD003 is single-condition, but the two-fault regime needs sharper expert selection.
# FD004 combines multiple operating conditions with multiple fault modes.
# This V12 version removes the Small branch from final fusion and uses a
# dual-expert gate over only Large and Corrected predictions.
...
# -----------------------------------------------------------
# Data loading utilities for FD004
# -----------------------------------------------------------

def load_fd004(path: str) -> pd.DataFrame:
...
class FIGLayer(nn.Module):
    """
    FD004-aware FIG layer:
    - first 3 features are operating conditions: os1, os2, os3
    - remaining features are sensor channels
    - the operating-condition summary is embedded and fused with sensor statistics
    """
    def __init__(self, input_dim: int, window_size: int, granule_dim: int = 64, op_dim: int = 3, op_embed_dim: int = 16):
        super().__init__()
        self.op_dim = op_dim
        sensor_dim = input_dim - op_dim
        if sensor_dim <= 0:
            raise ValueError(f"FIGLayer expected input_dim > {op_dim}, got {input_dim}")
        self.sensor_proj = nn.Linear(3 * sensor_dim + op_embed_dim, granule_dim)
        self.op_embed = nn.Sequential(
            nn.Linear(op_dim, op_embed_dim),
            nn.ReLU(),
        )

    def forward(self, x):
        op_x = x[:, :, :self.op_dim]
        sens_x = x[:, :, self.op_dim:]

        mean_t = sens_x.mean(dim=1)
        std_t = sens_x.std(dim=1)
        trend = sens_x[:, -1, :] - sens_x[:, 0, :]

        op_summary = op_x.mean(dim=1)
        op_feat = self.op_embed(op_summary)

        granule_raw = torch.cat([mean_t, std_t, trend, op_feat], dim=-1)
        return torch.relu(self.sensor_proj(granule_raw))
...
class ResidualCorrector(nn.Module):
    def __init__(self, granule_dim: int, feat_l_dim: int, hidden_dim: int = 64):
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),  # Prevent overfitting to training noise
            nn.Linear(hidden_dim, 1)
        )
...
class SharpenedExpertGating(nn.Module):
    ...
    def forward(self, y_l, y_c, g, conf_l):
        # Disagreement-aware gating inputs
        d_lc = torch.abs(y_l - y_c).unsqueeze(-1)

        gate_in = torch.cat([g, conf_l.unsqueeze(-1), d_lc], dim=-1)
        logits = self.gate_net(gate_in)

        # Sharpened softmax
        weights = torch.softmax(logits / self.temp, dim=-1)

        # Hard-threshold safe-switch gating:
        # experts below 0.15 are ejected and the rest are renormalized.
        threshold = 0.25
        mask = (weights >= threshold).float()
        masked_weights = weights * mask
        denom = masked_weights.sum(dim=-1, keepdim=True)

        # Fallback: if all experts are masked out (rare), revert to original weights
        safe_weights = torch.where(denom > 0, masked_weights / (denom + 1e-8), weights)
        ...
        # Minimal refinement freedom in the dual-expert setup.
        refine = torch.tanh(self.refine_net(g).squeeze(-1)) * self.refine_scale
        return y_weighted + refine, safe_weights, logits
...
class GFCoLLM(nn.Module):
    def __init__(self, input_dim: int, window_size: int):
        ...
        self.fusion = SharpenedExpertGating(granule_dim=granule_dim, temperature=0.1, refine_scale=0.1)
    ...
    def forward(self, x, y_true=None):
        ...
        y_fused, gate_weights, gate_logits = self.fusion(y_l, y_c, g, conf_l)
...
def main():
    ...
    train_df = load_fd004(TRAIN_FILE)
    test_df = load_fd004(TEST_FILE)
    ...
    feature_cols = ["os1", "os2", "os3"] + sensor_cols
    ...
    print("Evaluating enhanced GF-CoLLM on FD004 test set...")
