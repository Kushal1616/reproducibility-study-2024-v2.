...
# =========================================================
# Combined H=5 multi-horizon coverage script for FD001-004
...
def build_multihorizon_windows(df, feature_cols, window_size=30, horizon=5):
    X_list, Y_list = [], []
...
def run_one_dataset(name, cfg):
    ...
    os.makedirs("coverage_outputs_h5", exist_ok=True)
    np.save(f"coverage_outputs_h5/{name}_test_preds.npy", test_preds)
    np.save(f"coverage_outputs_h5/{name}_test_trues.npy", test_trues)
    np.save(f"coverage_outputs_h5/{name}_lower.npy", lower)
    np.save(f"coverage_outputs_h5/{name}_upper.npy", upper)
    np.save(f"coverage_outputs_h5/{name}_widths.npy", widths)
...
def main():
    ...
    ordered_cols = [
        "dataset", "nominal_coverage", "avg_mae", "avg_rmse",
        "final_h_rmse", "avg_coverage", "final_h_coverage"
    ]
    ...
    os.makedirs("coverage_outputs_h5", exist_ok=True)
    out_csv = "coverage_outputs_h5/coverage_summary_h5_all_datasets.csv"
