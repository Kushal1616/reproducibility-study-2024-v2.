...
# =========================================================
# Combined H=20 multi-horizon coverage script for FD001-004
# Coverage uses horizon-wise split-conformal intervals
# calibrated on a validation split.
# =========================================================
...
def build_multihorizon_windows(df, feature_cols, window_size=30, horizon=20):
    X_list, Y_list = [], []
...
def run_one_dataset(name, cfg):
    ...
    os.makedirs("coverage_outputs_h20", exist_ok=True)
    np.save(f"coverage_outputs_h20/{name}_test_preds.npy", test_preds)
    np.save(f"coverage_outputs_h20/{name}_test_trues.npy", test_trues)
    np.save(f"coverage_outputs_h20/{name}_lower.npy", lower)
    np.save(f"coverage_outputs_h20/{name}_upper.npy", upper)
    np.save(f"coverage_outputs_h20/{name}_widths.npy", widths)
...
def main():
    ...
    os.makedirs("coverage_outputs_h20", exist_ok=True)
    out_csv = "coverage_outputs_h20/coverage_summary_h20_all_datasets.csv"
