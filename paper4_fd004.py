...
# -*- coding: utf-8 -*-
"""
AutoTimes-style model for RUL prediction on NASA CMAPSS FD004.

Core ideas:
- Segment-based "tokens" along the time axis
- SegmentEmbedding MLP that maps each segment into the GPT-2 latent space
- Frozen GPT-2 decoder-only backbone
- Trainable segment embedding, position embeddings, and regression head

Adapted to regress RUL (scalar) instead of forecasting.
...
- Saves test predictions to `autotimes_fd004_predictions.csv`.
"""
...
def main():
    ...
    out_path = os.path.join(DATA_DIR, "autotimes_fd004_predictions.csv")
